<?php
class ModelTongkartPriceHistory extends Model {
	...
	public function getProductsSeller($product_id) {
		$query = $this->db->query("SELECT seller_id FROM " . DB_PREFIX . "seller_mapping WHERE product_id = '" . (int)$product_id . "'");

		return $query->row;
	}

	public function getProducts($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) RIGHT JOIN " . DB_PREFIX . "amazon_product_listing pl on(pl.product_id = p.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and pl.amazon_status = 'listed'";
		...
		if (isset($data['filter_order_sites']) && $data['filter_order_sites'] != 0) {
                    $sql .= " AND pl.customer_id = '".$data['filter_order_sites']."'";
                }
	...
	public function getTotalProducts($data = array()) {
		$sql = "SELECT COUNT(p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) RIGHT JOIN " . DB_PREFIX . "amazon_product_listing pl on(pl.product_id = p.product_id)";

		$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and pl.amazon_status = 'listed'";
	...
	public function getListingPrice($product_id,$price) {
        ...
        public function getPriceUpdatedDate($customer_id,$product_id) {
                $query = $this->db->query("SELECT date_modified  FROM " . DB_PREFIX . "tongkart_price_settings WHERE customer_id = '" . (int)$customer_id . "' and product_id = '".$product_id."'");
            if ($query->num_rows) {
                return $query->row['date_modified'];
                
            } else{
                $query = $this->db->query("SELECT date_modified FROM " . DB_PREFIX . "tongkart_price_settings WHERE customer_id = '0' and product_id = '".$product_id."'");
                if ($query->num_rows) {
                    return $query->row['date_modified'];
                } else{
                    $query = $this->db->query("SELECT date_updated  FROM " . DB_PREFIX . "admin_price_settings");
        ...
        public function getPriceUpdated_by($customer_id,$product_id) {
                $query = $this->db->query("SELECT updated_by  FROM " . DB_PREFIX . "tongkart_price_settings WHERE customer_id = '" . (int)$customer_id . "' and product_id = '".$product_id."'");
            if ($query->num_rows) {
                return $query->row['updated_by'];
                
            } else{
                $query = $this->db->query("SELECT updated_by FROM " . DB_PREFIX . "tongkart_price_settings WHERE customer_id = '0' and product_id = '".$product_id."'");
                if ($query->num_rows) {
                    return $query->row['updated_by'];
                } else{
