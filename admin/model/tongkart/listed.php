<?php
class ModelTongkartListed extends Model {
	...
	public function getProducts($data = array()) {
		$sql = "SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "amazon_product_listing al on (al.product_id = p.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and al.amazon_status = 'listed'";
	...
	public function getTotalProducts($data = array()) {
		$sql = "SELECT COUNT(DISTINCT al.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "amazon_product_listing al on (al.product_id = p.product_id)";

		$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and al.amazon_status = 'listed'";
	...
	public function getListingPrice($product_id,$price) {
        ...
        public function UpdateStatus($status) {
                $this->db->query("UPDATE `" . DB_PREFIX . "amazon_product_listing` SET  amazon_status = 'delete',listing_status_id = '1' where product_id =".$status);
                $this->db->query("UPDATE `" . DB_PREFIX . "product` SET  amazon_status = 'restricted' where product_id =".$status);
	}
}
