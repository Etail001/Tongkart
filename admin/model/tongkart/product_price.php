<?php
class ModelTongkartProductPrice extends Model {
	...
	public function getListingPrice($product_id,$price) {
        ...
        public function UpdateStatus($customer_id,$product_id,$cur_price,$up_price,$user_id) {
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tongkart_price_settings WHERE customer_id = '" . (int)$customer_id . "' and product_id = '".$product_id."'");
            if ($query->num_rows) {
                $up_price = $cur_price + $up_price;
                $this->db->query("UPDATE `" . DB_PREFIX . "tongkart_price_settings` SET  current_price = '".$cur_price."',updated_price = '".$up_price."', updated_by = ".$user_id." where price_id =".$query->row['price_id']);
                
            } else {
                $up_price = $cur_price + $up_price;
                $this->db->query("INSERT into `" . DB_PREFIX . "tongkart_price_settings` SET  current_price = '".$cur_price."',updated_price = '".$up_price."', product_id =".$product_id.", customer_id = ".$customer_id.", updated_by = ".$user_id);
            }
            if ($customer_id == 0) {
                $this->db->query("UPDATE `" . DB_PREFIX . "amazon_product_listing` SET  amazon_price_update = '0' where product_id =".$product_id);
            } else{
                $this->db->query("UPDATE `" . DB_PREFIX . "amazon_product_listing` SET  amazon_price_update = '0' where product_id =".$product_id." and customer_id = ".$customer_id);
            }    
	}
