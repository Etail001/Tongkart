<?php
class ModelTongkartSettings extends Model {
	...
	public function getCustomer($customer_id) {
		...
		$query = $this->db->query("SELECT e.order_id,e.reason,e.status,ed.enquiry_id,ed.description,ed.user,ed.date_added FROM " . DB_PREFIX . "order_enquiry_details ed left join " . DB_PREFIX . "order_enquiry e on (e.id = ed.enquiry_id) WHERE e.id = '" . (int)$enquiry_id . "' order by ed.date_added");
	...
	public function getCustomers($data = array()) {
		$sql = "SELECT e.*, CONCAT(c.firstname, ' ', c.lastname) AS name,o.temp_id FROM " . DB_PREFIX . "order_enquiry e left join " . DB_PREFIX . "order o on (o.order_id = e.order_id) left join " . DB_PREFIX . "customer c on (o.customer_id = c.customer_id)";
		...
		if (!empty($data['filter_date_added'])) {
			$implode[] = "DATE(e.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}
	...
	public function getTotalCustomers($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "order_enquiry e left join " . DB_PREFIX . "order o on (o.order_id = e.order_id) left join " . DB_PREFIX . "customer c on (o.customer_id = c.customer_id)";
	...
	public function getRewards($customer_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT order_id,temp_id,amazon_order_id FROM " . DB_PREFIX . "order WHERE order_id = '" . (int)$order_id . "'");
	...
	public function deleteLoginAttempts($email) {
        ...
        public function getCustomerByOrder($order_id) {
            $query_enquiry = $this->db->query("SELECT c.firstname, c.lastname FROM " . DB_PREFIX . "order o left join " . DB_PREFIX . "customer c on (c.customer_id = o.customer_id) WHERE o.order_id = '" . (int)$order_id . "'");
                if ($query_enquiry->num_rows){
                    return $query_enquiry->rows;
                } else{
                    return false;
                }
        }
        public function updateEnquiry($data,$customer_id) {
               if ($data['post_reply_status'] == 0){
                   $this->db->query("Update " . DB_PREFIX . "order_enquiry set status = '0' where id = ".$data['enquiry_id']);
               }
               $this->db->query("INSERT INTO " . DB_PREFIX . "order_enquiry_details set enquiry_id = '".$data['enquiry_id']."',description = '".$this->db->escape($data['postreplymessage'])."', user = '".$customer_id."'");
	}
        public function updateAdminPrice($data) {
               $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "admin_price_settings`"); 
               if ($query->num_rows){
                   $this->db->query("Update " . DB_PREFIX . "admin_price_settings set logistic_kg = '".$data['logistic_kg']."',last_logistic_price = '".$data['last_logistic_price']."', cb_ship_cost = '".$data['cb_ship_cost']."', qc_ship_cost = '".$data['qc_ship_cost']."' where price_id = ".$query->row['price_id']);
                   $this->db->query("Update " . DB_PREFIX . "product set price_updated = '0'");
                   $this->db->query("Update " . DB_PREFIX . "amazon_product_listing set amazon_price_update = '0' where amazon_status = 'listed'");
               } else{
                    $this->db->query("INSERT INTO " . DB_PREFIX . "admin_price_settings set logistic_kg = '".$data['logistic_kg']."',last_logistic_price = '".$data['last_logistic_price']."', cb_ship_cost = '".$data['cb_ship_cost']."', qc_ship_cost = '".$data['qc_ship_cost']."'");
               }
	}
