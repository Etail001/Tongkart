<?php
class ModelTongkartProductEnquiry extends Model {
	...
	public function getCustomer($customer_id) {
		...
		$query = $this->db->query("SELECT e.customer_id,e.reason,e.status,ed.enquiry_id,ed.description,ed.user,ed.date_added FROM " . DB_PREFIX . "product_enquiry_details ed left join " . DB_PREFIX . "product_enquiry e on (e.id = ed.enquiry_id) WHERE e.id = '" . (int)$enquiry_id . "' order by ed.date_added");
	...
	public function getCustomers($data = array()) {
		$sql = "SELECT e.*, CONCAT(c.firstname, ' ', c.lastname) AS name FROM " . DB_PREFIX . "product_enquiry e left join " . DB_PREFIX . "customer c on (e.customer_id = c.customer_id)";
	...
	public function getTotalCustomers($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_enquiry e left join " . DB_PREFIX . "customer c on (e.customer_id = c.customer_id)";
	...
	public function deleteLoginAttempts($email) {
        ...
        public function updateEnquiry($data,$customer_id) {
               if ($data['post_reply_status'] == 0){
                   $this->db->query("Update " . DB_PREFIX . "product_enquiry set status = '0' where id = ".$data['enquiry_id']);
               }
               $this->db->query("INSERT INTO " . DB_PREFIX . "product_enquiry_details set enquiry_id = '".$data['enquiry_id']."',description = '".$this->db->escape($data['postreplymessage'])."', user = '".$customer_id."'");
	}
