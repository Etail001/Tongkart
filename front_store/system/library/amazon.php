...
class AmazonService extends AmazonDB {
     ...
     * @param Object $data
     */
    public function checkSellerAccount($data) {
        $timestamp = gmdate("Y-m-d\TH:i:s\Z");
        $this->UserAgent = '<MWS_ListMarketplaceParticipations>/<1.0> (Language=PHP/' . phpversion() . ')';
        $this->path = '/Sellers/2011-07-01?AWSAccessKeyId=AKIAJBIBQUN2FA3Z6JGQ';
        ...
        $response = $this->SubmitRequest($this->end_point,'',$params, $signature, $data['amazon_store_name']);
        echo "<pre>";print_r($response);echo "</pre>1111";die;
        return $response;
    }
     ...
     */
    public function submitInventoryFeed($storeInfo, $filePath, $type) {
    ...
    public function getFeedResponse($storeInfo, $Type = '', $feedID) {
        $timestamp = gmdate("Y-m-d\TH:i:s\Z");
        $this->UserAgent = '<MWS_SubmitFeed>/<1.0> (Language=PHP/' . phpversion() . ')';
        $this->path = '/?';
        $this->contentType = 'text/xml';
     ...
     * @param String $type
     */
    public function submitProductFeed($storeInfo, $filePath, $type) {
        $db = new AmazonDB();
        $timestamp = gmdate("Y-m-d\TH:i:s\Z");
        $this->UserAgent = '<MWS_SubmitFeed>/<1.0> (Language=PHP/' . phpversion() . ')';
        $this->path = '/?';
     ...
     * @param String $type
     */
    public function submitPriceFeed($storeInfo, $filePath, $type) {
     ...
     * @param String $type
     */
    public function submitImageFeed($storeInfo, $filePath, $type) {
        $db = new AmazonDB();
        $timestamp = gmdate("Y-m-d\TH:i:s\Z");
        $this->UserAgent = '<MWS_SubmitFeed>/<1.0> (Language=PHP/' . phpversion() . ')';
        $this->path = '/?';
    ...
    public function ListOrder($storeInfo) {
...
//echo "Parsed result"; print_R($parsed_result);
    ...
    public function ListOrderItems($storeInfo, $SellerCentral, $AmazonOrderId) {
    ...
    public function ShippingConfirmation($storeInfo, $xml_file) {
        $timestamp = gmdate("Y-m-d\TH:i:s\Z");
        $AWSAccessKeyId = $storeInfo['aws_key_id'];
        $MerchantId = $storeInfo['merchant_id'];
        $Marketplace = $storeInfo['market_place_id'];
