...
class ControllerCrmContact extends Controller {
	...
	public function index() {
		...
		$data['cancel'] = $this->url->link('crm/contact', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('crm/contact', $data));
	}
	...
	function add()
	{
		...
		$this->response->setOutput($this->load->view('crm/add', $data));
	}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$json) {
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('crm/contact/send', 'user_token=' . $this->session->data['user_token'] . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
