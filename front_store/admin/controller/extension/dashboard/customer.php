...
class ControllerExtensionDashboardCustomer extends Controller {
	...
	public function dashboard() {
		...
		$today = $this->model_sale_order->getTotalOrdersShipped(array('filter_date_added' => date('Y-m-d', strtotime('-1 day'))));

		$yesterday = $this->model_sale_order->getTotalOrdersShipped(array('filter_date_added' => date('Y-m-d', strtotime('-2 day'))));
		...
		$sale_total = $this->model_sale_order->getTotalOrdersShipped();
		...
		if($user_id == '4')
		{
			$data['customer'] = $this->url->link('cnwms/cnwms/shippedOrder', 'user_token=' . $this->session->data['user_token'], true);
		}
		else
		{
			$data['customer'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'], true);
		}
