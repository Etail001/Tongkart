<?php
class ControllerExtensionDashboardShipment extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('extension/dashboard/shipment');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/dashboard/shipment', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/dashboard/shipment', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/dashboard/shipment_form', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/dashboard/shipment')) {
	...
	public function dashboard() {
		...
		if($user_id == '4')
		{
			$data['order'] = $this->url->link('cnwms/shipment', 'user_token=' . $this->session->data['user_token'], true);
		}
		else
		{
		...
		return $this->load->view('extension/dashboard/shipment_info', $data);
	}
}
