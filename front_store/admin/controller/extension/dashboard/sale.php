...
class ControllerExtensionDashboardSale extends Controller {
	...
	public function dashboard() {
		...
		$today = $this->model_sale_order->getTotalOrdersProcessed(array('filter_date_added' => date('Y-m-d', strtotime('-1 day'))));

		$yesterday = $this->model_sale_order->getTotalOrdersProcessed(array('filter_date_added' => date('Y-m-d', strtotime('-2 day'))));
		...
		$sale_total = $this->model_sale_order->getTotalOrdersProcessed();
		...
		if($user_id == '4')
		{
			$data['sale'] = $this->url->link('cnwms/cnwms/processedOrder', 'user_token=' . $this->session->data['user_token'], true);
		}
		else
		{
