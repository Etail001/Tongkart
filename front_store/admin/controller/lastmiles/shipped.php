<?php
class ControllerLastmilesShipped extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('lastmiles/lastmiles');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('lastmiles/shipped');
	...
	protected function getList() {
		...
		$data['action'] = $this->url->link('lastmiles/lastmiles/generateLabel', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$order_total = $this->model_lastmiles_shipped->getTotalOrders($filter_data);

		$results = $this->model_lastmiles_shipped->getOrders($filter_data);
		//echo '<pre>';
		//print_r($results);die;
		
		foreach ($results as $result) {
			$label = 'http://'.$result['label'];
			$data['orders'][] = array(
				'order_id'      => $result['order_id'],
				'order_status'  => $result['name'] ? $result['name'] : $this->language->get('text_missing'),
		...
		$this->response->setOutput($this->load->view('lastmiles/shipped_list', $data));
	}
	...
	function generateLabel()
	{
		...
		if (isset($this->request->post['selected'])) {
			...
			$method = $this->request->post['logistics'];
		...
		if($method == 'EcomExpress')
			{
				...
				$paramsorder = array
							...
							'HeaderRequest' => array
								(
									'appkey' => 'DYhG93b0qyJfIxfs2yuVoUubWwvniR2G0FgaC9mi',
									'token' => '76859309658453542496749683645'
								),
							'addOrderRequestInfoArray' => array
								(
									'shippingMode' => '', //Or AirCargo
									'customerOrderNo' => 'INF836578465',//$dataFilter[$j]['data'],
									'Tins' => '07AACCF8995N1ZQ',
									'shipperAddresstype' => '2',
									'shipperName' => 'FADA TRADING PVT LTD',
									'shipperEmail' => 'lilanand.jha@fadatd.com',
									'shipperAddress' => 'S/74, Hissa No. 11 & 12, Near Rajlaxmi warehouse Bhiwandi Nasik Highway, Vadpa Village Dist. Thane -421302, Maharastra',
									'shipperPhone' => '8291251929',
									'shipperPostcode' => '421302',
									'shipperCompanyName' => 'FADA TRADING PVT LTD',
									'shippingMethod' => 'ecomexpress',
									'shipperCountryCode' => 'IN',
									'shipperCountryName' => 'INDIA',
									'recipientCountry' => 'India',
									'recipientCountryCode' => 'IN',
									'shipperCity' => 'Maharastra',
									'shipperStateOrProvinceCode' => 'MH',
									'recipientName' => 'rajat',
									'recipientState' => '',
									'recipientStateOrProvinceCode' => 'DL',
									'recipientCity' => 'Gurgaon',
									'recipientAddress' =>'delhi',
									'recipientZipCode' => '110037',
									'recipientPhone' => trim('9205103665'),
									'recipientEmail' => '',
						...
						$url = "http://54.211.226.195/wms/AwbgenerateTests/addOrderRquest";
						...
						$awbDetails['response'] = $data;
						$awbDetails['request'] = $paramsorder;
						
						$this->saveResponse($awbDetails);
	...
	{
		$this->load->model('lastmiles/lastmiles');
		$order_detail = $this->model_lastmiles_lastmiles->insertLabelDetails($data);
	}
