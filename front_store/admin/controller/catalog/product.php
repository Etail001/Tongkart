...
class ControllerCatalogProduct extends Controller {
	...
	public function listonflipkart()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			for($i=0;$i<$productCount;$i++){
				$result = $this->model_catalog_product->getProduct($this->request->post['selected'][$i]);
				//echo '<pre>';
				//print_r($result);die;
				$token = $this->getFKAccessToken();
	...
	protected function getList() {
		...
		$data['flipkart'] = $this->url->link('catalog/product/listonflipkart', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['paytm'] = $this->url->link('catalog/product/listonpaytm', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			$seller = $this->model_catalog_product->getProductsSeller($result['product_id']);
			...
			//$listing_price = $this->model_catalog_product->getListingPrice($result['product_id'],$result['price']);
			$data['products'][] = array(
				...
				'listing_price' => $this->currency->format($result['listing_price'], $this->config->get('config_currency')),
				'special'    => $special,
	...
	protected function getForm() {
		...
		$this->load->model('tool/image');
		$seller = $this->model_catalog_product->getProductsSeller($this->request->get['product_id']);
                if($seller['seller_id']){
