...
class ModelAmazonAmazon extends Model {
    public function getImageProductDetails($product_id,$seller_id) {
        $sql = "SELECT p.product_id from " . DB_PREFIX . "product p left join " . DB_PREFIX . "seller_mapping sm on(sm.product_id = p.product_id) where sm.seller_id = ".$seller_id." and p.amazon_status = 'listed' and p.amazon_image_update = '0'";
...
<AmazonEnvelope
	...
	<Header>
		...
		if($seller_id == '2'){
			...
			$string .= '<MerchantIdentifier>A3EXBLBBCU4DIN</MerchantIdentifier>';
		}
	$string .= '</Header>
                      ...
                      if ($query->num_rows) {
                       $order_total = $this->getOrderTotal($product,$query->rows[0]['shipping_method']);
                          ...
                          if ($query_product->num_rows == 0) {
                           $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', OrderItemId = '" . $this->db->escape($product['OrderItemId']) . "', product_id = '" . (int)$product_id . "', name = '" . $this->db->escape($product['Title']) . "', model = '" . $this->db->escape($model) . "', quantity = '" . (int)$product['ProductInfo']['NumberOfItems'] . "', price = '" . (float)$product['ItemPrice']['Amount'] . "', total = '" . (float)$total . "', tax = '" . (float)$product['ItemTax']['Amount'] . "', reward = '0'");
                    ...
                    $query = $this->db->query("SELECT order_id,shipping_method FROM " . DB_PREFIX . "order WHERE amazon_order_id = '".$amazon_order_id."'");
                      if ($query->num_rows) {
                           foreach($product as $products){
                               ...
                               if ($query_product->num_rows == 0) {
                                $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', OrderItemId = '" . $this->db->escape($products['OrderItemId']) . "', product_id = '" . (int)$product_id . "', name = '" . $this->db->escape($products['Title']) . "', model = '" . $this->db->escape($model) . "', quantity = '" . (int)$products['ProductInfo']['NumberOfItems'] . "', price = '" . (float)$products['ItemPrice']['Amount'] . "', total = '" . (float)$total . "', tax = '" . (float)$products['ItemTax']['Amount'] . "', reward = '0'");
