...
class ControllerExtensionModuleDealproduct extends Controller {
	public function index($setting) {
			...
			foreach ($products as $product_id) {
				...
				if ($product_info) {
					if ($product_info['image']) {
						$image = $product_info['image'];//$this->model_tool_image->resize($product_info['image'], $setting['width'], $setting['height']);
					} else {
					...
					if ($this->config->get('config_review_status')) {
					 ...
					 /*Additional images start*/
                            ...
                            foreach ($results as $result){
                                    $more_images['images'][]=array(
                                        'popup_more' => $result['image']//$this->model_tool_image->resize($result['image'],$setting['width'], $setting['height'])
                                    );
                                    //print_r($more_images);
                            }
		...
		if ($data['products']) {
			return $this->load->view('extension/module/dealproduct', $data);
		}
	}
}
