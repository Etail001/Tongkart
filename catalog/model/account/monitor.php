<?php
class ModelAccountMonitor extends Model {
	...
	public function getApiIps($api_id) {
        ...
        public function getTotalMonitorStatus($customer_id) {
                $sql = "SELECT count(*) as total FROM " . DB_PREFIX . "all_amazon_feeds where customer_id = ".$customer_id." and type != 'inventory'";
		$query = $this->db->query($sql);

		return $query->row['total'];
	}
        public function getMonitorStatus($data,$customer_id) {
                $sql = "SELECT * FROM " . DB_PREFIX . "all_amazon_feeds where customer_id = ".$customer_id." and type != 'inventory'";
