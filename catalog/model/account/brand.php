...
class ModelAccountBrand extends Model {
	...
	public function getApiIps($api_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "amazon_brand_settings where customer_id = ".$customer_id." and category_id = '".$data['category']."'");
                if ($query->num_rows){
                    $this->db->query("UPDATE `" . DB_PREFIX . "amazon_brand_settings` SET text = '" . $this->db->escape($data['text']) . "' where brand_id =".$query->rows[0]['brand_id']);
                } else{
                    $this->db->query("INSERT INTO `" . DB_PREFIX . "amazon_brand_settings` SET customer_id = '" . (int)$customer_id . "', category_id = '" . $this->db->escape($data['category']) . "', text = '" . $this->db->escape($data['text']) . "'");
                }
        ...
        public function getTotalBrandSet($customer_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "amazon_brand_settings where customer_id = ".$customer_id);

		return $query->row['total'];
	}
        public function getBrandSet($data,$customer_id) {
                $sql = "SELECT * FROM " . DB_PREFIX . "amazon_brand_settings s left join " . DB_PREFIX . "category_description cd on (s.category_id = cd.category_id) where customer_id = ".$customer_id;
