...
class ModelAccountCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', user_id = '" . $this->db->escape($data['user_id']) . "', company = '" . $this->db->escape($data['company']) . "', gstin = '" . $this->db->escape($data['gstin']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
	...
	public function getCustomer($customer_id) {
        ...
        public function checkUserId($user_id,$customer_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id != '" . (int)$customer_id . "' and user_id = '".$user_id."'");
                if ($query->num_rows) {
                    return true;
                } else{
                    return false;
