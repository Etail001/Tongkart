...
class ModelChinaboardChinaboard extends Model {
	...
	public function updateOrder($data,$order_id)
    {
		...
		//print_r($data);die;
	     if(isset($data['msg'][$order_id]['msg']) && $data['msg'][$order_id]['msg'] == 'success'){
                ...
                $this->db->query("INSERT INTO " . DB_PREFIX . "supplire_orders SET supplire_order_id = '".$order_id."',
                order_id = '".$order_id."',
                wms_order_id = '".$order_id."',info='".$data['msg'][$order_id]['msg']."',status='Success',seller_id='1'"); 
	...
	public function getOrderDetails($data)
	{
		...
		for($i=0;$i<$countCategory;$i++)
		{		
			$sn = $this->db->query("SELECT * from " . DB_PREFIX . "supplire_order_import WHERE item_id = '" . $this->db->escape($data['msg']['page_result'][$i]['order_sn']) . "'");
			//echo '<pre>';
			//print_r($sn);die;
			
			if($sn->num_rows == 0){
				
				//$sn = $this->db->query("SELECT * from " . DB_PREFIX . "order WHERE temp_id = '" . $this->db->escape($data['msg']['page_result'][$i]['user_order_sn']) . "'");
				//echo '<pre>';
				//print_r($sn);die;
				$this->db->query("UPDATE " . DB_PREFIX . "supplire_orders SET supplire_order_id = '".$this->db->escape($data['msg']['page_result'][$i]['order_id'])."' where
                wms_order_id = '".$data['msg']['page_result'][$i]['user_order_sn']."'"); 
                
				$this->db->query("INSERT INTO " . DB_PREFIX . "supplire_order_import SET supplire_order_id = '" . $this->db->escape($data['msg']['page_result'][$i]['order_id']) . "', 
				item_id = '" . $this->db->escape($data['msg']['page_result'][$i]['user_order_sn']) . "',
				ship_confirm_date = '" . $this->db->escape($data['msg']['page_result'][$i]['add_time']) . "',
				order_no = '" . $this->db->escape($data['msg']['page_result'][$i]['order_id']) . "',
				track_number = '" . $this->db->escape($data['msg']['page_result'][$i]['order_sn']) . "',
				track_updated = '" . $this->db->escape($data['msg']['page_result'][$i]['shipping_time']) . "',
				shipping_cost = '" . $this->db->escape($data['msg']['page_result'][$i]['shipping_fee']) . "',
				supp_order_id = '" . $this->db->escape($data['msg']['page_result'][$i]['order_id']) . "',
				order_status = '" . $this->db->escape($data['msg']['page_result'][$i]['order_status']) . "',
				sku = '" . $this->db->escape($data['msg']['page_result'][$i]['goods_list'][0]['goods_sn']) . "',
				sku_qty = '" . $this->db->escape($data['msg']['page_result'][$i]['goods_list'][0]['goods_number']) . "',
				shipping_method = '" . $this->db->escape($data['msg']['page_result'][$i]['shipping_name']) . "'");
			}
