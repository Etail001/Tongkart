<?php
class ControllerAccountApi extends Controller {
	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$this->model_account_api->editSettings($this->request->post,$this->customer->getId());
		...
		$data['action'] = $this->url->link('account/api', '', true);
		...
		$data['common'] = $this->load->controller('account/common');
                $total_products = $this->model_account_api->getTotalProducts();
                $data['total_product'] = $total_products;
                $total_new_products = $this->model_account_api->getTotalNewProducts();
                $data['total_new_products'] = $total_new_products;
                $total_listed_products = $this->model_account_api->getTotalListedProducts($this->customer->getId());
                $data['total_listed_product'] = $total_listed_products;
                $total_restricted_products = $this->model_account_api->getTotalRestrictedProducts($this->customer->getId());
                $data['total_restricted_products'] = $total_restricted_products;
                $total_error_products = $this->model_account_api->getTotalErrorProducts($this->customer->getId());
                $data['total_error_products'] = $total_error_products;
                $data['total_not_listed_products'] = $total_products -$total_listed_products;
                $settings = $this->model_account_api->getSettings($this->customer->getId());
                if (!empty($settings)){
		...
		$this->response->setOutput($this->load->view('account/api', $data));
	}
        public function update_status(){
            $status = $this->request->get['id'];
            $this->load->model('account/api');
            $this->model_account_api->UpdateStatus($status,$this->customer->getId());
        }
}