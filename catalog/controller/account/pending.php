<?php
class ControllerAccountPending extends Controller {
	public function index() {
		...
		$this->load->language('account/pending');
                $this->load->model('tool/image');

		$this->document->setTitle($this->language->get('heading_title'));
                $this->load->model('account/pending');
		...
		$data['action'] = $this->url->link('account/pending', '', true);
		...
		$data['back'] = $this->url->link('account/account', '', true);
                ...
                $data['url'] = $this->url->link('account/pending',$url);
                $data['page'] = $page;
                $data['limit'] = $limit;
		...
		$data['common'] = $this->load->controller('account/common');
                ...
                $total_order_count = $this->model_account_pending->getTotalOrders($filter_data,$this->customer->getId());
                $total_orders = $this->model_account_pending->getorders($filter_data,$this->customer->getId());
                ...
                foreach($total_orders as $list){
                    ...
                    $tongkart_price = $this->model_account_pending->getTongkartOrderPrice($list['order_id']);
                ...
                $pagination->url = $this->url->link('account/pending',$url.'&page={page}');
		...
		$this->response->setOutput($this->load->view('account/pending', $data));
	}
		...
		if (isset($this->request->post['selected'])) {
                        $this->model_account_pending->ListAmazon($this->request->post['selected'],$this->customer->getId());
                        $this->session->data['success'] = 'We have Initiated listing of those products. It Usually takes 5-10 min to get them listed.';
                        $this->response->redirect($this->url->link('account/pending'));
        ...
        public function export() {
            ...
            $export_product = $this->model_account_pending->getAllorders($filter_data,$this->customer->getId());
            ...
            foreach($export_product as $detail){
                $i++;
                $tongkart_price = $this->model_account_pending->getTongkartOrderPrice($detail['order_id']);
            ...
            $filename = strtotime("now").'export_pending_order.xls';
