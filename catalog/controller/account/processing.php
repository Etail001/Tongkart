<?php
class ControllerAccountProcessing extends Controller {
	public function index() {
		...
		$this->load->language('account/processing');
                $this->load->model('tool/image');

		$this->document->setTitle($this->language->get('heading_title'));
                $this->load->model('account/processing');
		...
		$data['action'] = $this->url->link('account/processing', '', true);
		...
		$data['back'] = $this->url->link('account/account', '', true);
                ...
                $data['url'] = $this->url->link('account/processing',$url);
                $data['page'] = $page;
                $data['limit'] = $limit;
		...
		$data['common'] = $this->load->controller('account/common');
                ...
                $total_order_count = $this->model_account_processing->getTotalOrders($filter_data,$this->customer->getId());
                $total_orders = $this->model_account_processing->getorders($filter_data,$this->customer->getId());
                ...
                foreach($total_orders as $list){
                    ...
                    $tongkart_price = $this->model_account_processing->getTongkartOrderPrice($list['order_id']);
                ...
                $pagination->url = $this->url->link('account/processing',$url.'&page={page}');

                $data['pagination'] = $pagination->render();
                $data['payment'] = $this->url->link('account/processing/payment');
		...
		$this->response->setOutput($this->load->view('account/processing', $data));
	}
        public function export() {
            ...
            $export_product = $this->model_account_processing->getAllorders($filter_data,$this->customer->getId());
            ...
            foreach($export_product as $detail){
                $i++;
                $tongkart_price = $this->model_account_processing->getTongkartOrderPrice($detail['order_id']);
		...
		$this->load->model('account/processing');
		if (isset($this->request->post['selected'])) {
                    $result = $this->model_account_processing->pay($this->request->post['selected'],$this->customer->getId());
                    if ($result == 'success'){
                        $this->session->data['success'] = 'Succesfully Paid from your Wallet. Orders will be processed soon.';
                        $this->response->redirect($this->url->link('account/processing'));
                    } else {
