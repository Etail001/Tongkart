<?php
class ControllerAccountOdrShipped extends Controller {
	public function index() {
		...
		$this->load->language('account/odr_shipped');
                $this->load->model('tool/image');

		$this->document->setTitle($this->language->get('heading_title'));
                $this->load->model('account/odr_shipped');
		...
		$data['action'] = $this->url->link('account/odr_shipped', '', true);
		...
		$data['back'] = $this->url->link('account/account', '', true);
                ...
                $data['url'] = $this->url->link('account/odr_shipped',$url);
                $data['page'] = $page;
                $data['limit'] = $limit;
		...
		$data['common'] = $this->load->controller('account/common');
                ...
                $total_order_count = $this->model_account_odr_shipped->getTotalOrders($filter_data,$this->customer->getId());
                $total_orders = $this->model_account_odr_shipped->getorders($filter_data,$this->customer->getId());
                ...
                foreach($total_orders as $list){
                    ...
                    $tongkart_price = $this->model_account_odr_shipped->getTongkartOrderPrice($list['order_id']);
                ...
                $pagination->url = $this->url->link('account/odr_shipped',$url.'&page={page}');
		...
		$this->response->setOutput($this->load->view('account/odr_shipped', $data));
	}
		...
		if (isset($this->request->post['selected'])) {
                        $this->model_account_odr_shipped->ListAmazon($this->request->post['selected'],$this->customer->getId());
                        $this->session->data['success'] = 'We have Initiated listing of those products. It Usually takes 5-10 min to get them listed.';
                        $this->response->redirect($this->url->link('account/odr_shipped'));
        ...
        public function export() {
            ...
            $export_product = $this->model_account_odr_shipped->getAllorders($filter_data,$this->customer->getId());
            ...
            foreach($export_product as $detail){
                $i++;
                $tongkart_price = $this->model_account_odr_shipped->getTongkartOrderPrice($detail['order_id']);
