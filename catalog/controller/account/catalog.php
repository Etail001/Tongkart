<?php
class ControllerAccountCatalog extends Controller {
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/catalog', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('account/catalog', '', true)
		);

		$data['action'] = $this->url->link('account/catalog', '', true);
		...
		$data['common'] = $this->load->controller('account/common');
                ...
                $pending_order = $this->model_account_catalog->getTotalPendingOrders($this->customer->getId());
                $data['pending_order'] = $pending_order;
                $cancel_order = $this->model_account_catalog->getTotalCancelOrders($this->customer->getId());
                $data['cancel_order'] = $cancel_order;
                $shipped_order = $this->model_account_catalog->getTotalShippedOrders($this->customer->getId());
                $data['shipped_order'] = $shipped_order;
                $total_order = $this->model_account_catalog->getTotalOrders($this->customer->getId());
                $data['total_order'] = $total_order;
                $total_products = $this->model_account_catalog->getTotalProducts();
                $data['total_product'] = $total_products;
                $total_new_products = $this->model_account_catalog->getTotalNewProducts();
                $data['total_new_products'] = $total_new_products;
                $total_listed_products = $this->model_account_catalog->getTotalListedProducts($this->customer->getId());
                $data['total_listed_product'] = $total_listed_products;
                $total_restricted_products = $this->model_account_catalog->getTotalRestrictedProducts($this->customer->getId());
                $data['total_restricted_products'] = $total_restricted_products;
                $total_error_products = $this->model_account_catalog->getTotalErrorProducts($this->customer->getId());
                $account_details = $this->model_account_catalog->getAccountDetails($this->customer->getId());
		...
		$this->response->setOutput($this->load->view('account/catalog', $data));
	}
}