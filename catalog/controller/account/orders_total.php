<?php
class ControllerAccountOrdersTotal extends Controller {
	public function index() {
		...
		$this->load->language('account/orders_total');
                $this->load->model('tool/image');

		$this->document->setTitle($this->language->get('heading_title'));
                $this->load->model('account/orders_total');
		...
		$data['action'] = $this->url->link('account/orders_total', '', true);
		...
		$data['back'] = $this->url->link('account/account', '', true);
                ...
                $data['url'] = $this->url->link('account/orders_total',$url);
                $data['page'] = $page;
                $data['limit'] = $limit;
		...
		$data['common'] = $this->load->controller('account/common');
                ...
                $total_order_count = $this->model_account_orders_total->getTotalOrders($filter_data,$this->customer->getId());
                $total_orders = $this->model_account_orders_total->getorders($filter_data,$this->customer->getId());
                ...
                foreach($total_orders as $list){
                    if ($list['order_status_id'] == 15){
                        ...
                        $status = 'Under Reveiw';
                    } else if ($list['order_status_id'] == 20){
                        $status = 'Awaiting Payment';
                    } else{
                        $status = 'Processing';
                    }
                    $tongkart_price = $this->model_account_orders_total->getTongkartOrderPrice($list['order_id']);
                ...
                $pagination->url = $this->url->link('account/orders_total',$url.'&page={page}');
		...
		$this->response->setOutput($this->load->view('account/orders_total', $data));
	}
		...
		if (isset($this->request->post['selected'])) {
                        $this->model_account_orders_total->ListAmazon($this->request->post['selected'],$this->customer->getId());
                        $this->session->data['success'] = 'We have Initiated listing of those products. It Usually takes 5-10 min to get them listed.';
                        $this->response->redirect($this->url->link('account/orders_total'));
        ...
        public function export() {
            ...
            $export_product = $this->model_account_orders_total->getAllorders($filter_data,$this->customer->getId());
            ...
            foreach($export_product as $detail){
                $i++;
                if ($detail['order_status_id'] == 15){
                    $status = 'Shipped';
                } else if ($detail['order_status_id'] == 1){
                    $status = 'Pending';
                } else if ($detail['order_status_id'] == 7){
                    $status = 'Canceled';
                } else if ($detail['order_status_id'] == 19){
                    $status = 'Under Reveiw';
                } else{
                    $status = 'Processing';
                }
                $tongkart_price = $this->model_account_orders_total->getTongkartOrderPrice($detail['order_id']);
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue("G$i","{$detail['date_added']}");
            ...
            $filename = strtotime("now").'export_order.xls';
